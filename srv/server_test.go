...
func TestFullCRUDFlow(t *testing.T) {
	...
	resp := doReq(t, ts, "POST", "/api/users", body, "")
	...
	// 2. Login
	resp = doReq(t, ts, "POST", "/api/users/"+fp+"/login", `{"password":"hunter2"}`, "")
	...
	// 3. PUT entry
	resp = doReqRaw(t, ts, "PUT", "/api/users/"+fp+"/entries/Email/gmail", []byte("encrypted-blob-data"), token)
	expectStatus(t, resp, http.StatusNoContent)

	// 4. List entries
	resp = doReq(t, ts, "GET", "/api/users/"+fp+"/entries", "", token)
	expectStatus(t, resp, http.StatusOK)
	var listResp struct {
		Entries []struct {
	...
	// 6. Move entry
	resp = doReq(t, ts, "POST", "/api/users/"+fp+"/entries/move", `{"from":"Email/gmail","to":"Email/gmail-moved"}`, token)
	expectStatus(t, resp, http.StatusNoContent)
	...
	// 7. Delete entry
	resp = doReq(t, ts, "DELETE", "/api/users/"+fp+"/entries/Email/gmail-moved", "", token)
	expectStatus(t, resp, http.StatusNoContent)
...
func TestUnauthorized(t *testing.T) {
	...
	// No token at all
	resp := doReq(t, ts, "GET", "/api/users/abc123/entries", "", "")
	expectStatus(t, resp, http.StatusUnauthorized)

	// Bad token
	resp = doReq(t, ts, "GET", "/api/users/abc123/entries", "", "bad-token")
	expectStatus(t, resp, http.StatusUnauthorized)
	...
	createUserAndGetToken := func(fp, pw string) string {
		...
		resp := doReq(t, ts, "POST", "/api/users", body, "")
		expectStatus(t, resp, http.StatusCreated)
		resp = doReq(t, ts, "POST", "/api/users/"+fp+"/login", `{"password":"`+pw+`"}`, "")
		expectStatus(t, resp, http.StatusOK)
	...
	resp = doReq(t, ts, "GET", "/api/users/userB/entries", "", tokenA)
...
func TestLoginWrongPassword(t *testing.T) {
	...
	// Create user
	resp := doReq(t, ts, "POST", "/api/users", `{"password":"correct","public_key":"pk1","fingerprint":"fp1"}`, "")
	expectStatus(t, resp, http.StatusCreated)

	// Wrong password
	resp = doReq(t, ts, "POST", "/api/users/fp1/login", `{"password":"wrong"}`, "")
	expectStatus(t, resp, http.StatusUnauthorized)
}
...
func TestTOTPFlow(t *testing.T) {
	...
	resp := doReq(t, ts, "POST", "/api/users", `{"password":"pw","public_key":"pk","fingerprint":"totp1"}`, "")
	expectStatus(t, resp, http.StatusCreated)

	resp = doReq(t, ts, "POST", "/api/users/totp1/login", `{"password":"pw"}`, "")
	expectStatus(t, resp, http.StatusOK)
	var lr map[string]string
	decodeJSON(t, resp, &lr)
	token := lr["token"]
	...
	// Setup TOTP
	resp = doReq(t, ts, "POST", "/api/users/totp1/totp/setup", "", token)
	expectStatus(t, resp, http.StatusOK)
	...
	resp = doReq(t, ts, "POST", "/api/users/totp1/totp/confirm", confirmBody, token)
	expectStatus(t, resp, http.StatusOK)
	...
	resp = doReq(t, ts, "POST", "/api/users/totp1/login/2fa",
...
func TestExportImport(t *testing.T) {
	...
	// Create user and login
	resp := doReq(t, ts, "POST", "/api/users", `{"password":"pw","public_key":"pk","fingerprint":"exp1"}`, "")
	expectStatus(t, resp, http.StatusCreated)
	resp = doReq(t, ts, "POST", "/api/users/exp1/login", `{"password":"pw"}`, "")
	expectStatus(t, resp, http.StatusOK)
	var lr map[string]string
	decodeJSON(t, resp, &lr)
	token := lr["token"]
	...
	// Create some entries
	resp = doReqRaw(t, ts, "PUT", "/api/users/exp1/entries/Email/gmail", []byte("blob1"), token)
	expectStatus(t, resp, http.StatusNoContent)
	resp = doReqRaw(t, ts, "PUT", "/api/users/exp1/entries/Social/github", []byte("blob2"), token)
	expectStatus(t, resp, http.StatusNoContent)
	...
	resp = doReq(t, ts, "POST", "/api/users", `{"password":"pw","public_key":"pk2","fingerprint":"imp1"}`, "")
	expectStatus(t, resp, http.StatusCreated)
	resp = doReq(t, ts, "POST", "/api/users/imp1/login", `{"password":"pw"}`, "")
	expectStatus(t, resp, http.StatusOK)
	...
	// Import the tar.gz
	resp = doReqRaw(t, ts, "POST", "/api/users/imp1/import", exportData, token2)
	expectStatus(t, resp, http.StatusOK)
	...
	resp = doReq(t, ts, "GET", "/api/users/imp1/entries", "", token2)
	expectStatus(t, resp, http.StatusOK)
	var listResp struct {
		Entries []struct {
			Path string `json:"path"`
...
func TestFingerprintFromKey(t *testing.T) {
	...
	resp := doReq(t, ts, "POST", "/api/users", `{"password":"pw","public_key":"my-public-key"}`, "")
	expectStatus(t, resp, http.StatusCreated)
...
func TestUpsertEntryUpdates(t *testing.T) {
	...
	// Create user and login
	resp := doReq(t, ts, "POST", "/api/users", `{"password":"pw","public_key":"pk","fingerprint":"upd1"}`, "")
	expectStatus(t, resp, http.StatusCreated)
	resp = doReq(t, ts, "POST", "/api/users/upd1/login", `{"password":"pw"}`, "")
	expectStatus(t, resp, http.StatusOK)
	var lr map[string]string
	decodeJSON(t, resp, &lr)
	token := lr["token"]
	...
	// Create entry
	resp = doReqRaw(t, ts, "PUT", "/api/users/upd1/entries/test", []byte("v1"), token)
	expectStatus(t, resp, http.StatusNoContent)

	// Update same path
	resp = doReqRaw(t, ts, "PUT", "/api/users/upd1/entries/test", []byte("v2"), token)
	expectStatus(t, resp, http.StatusNoContent)
	...
	resp = doReq(t, ts, "GET", "/api/users/upd1/entries", "", token)
	expectStatus(t, resp, http.StatusOK)
	var listResp struct {
