...
func (q *Queries) GetSessionInfo(ctx context.Context, fingerprint string) (GetSessionInfoRow, error) {
...
SELECT fingerprint, password_hash, public_key, totp_secret, totp_enabled, created, login_time, last_activity, gpg_id FROM users WHERE fingerprint = ?
`
...
const updateUserGpgID = `-- name: UpdateUserGpgID :exec
UPDATE users
SET gpg_id = ?
WHERE fingerprint = ?
`

type UpdateUserGpgIDParams struct {
	GpgID       *string `json:"gpg_id"`
	Fingerprint string  `json:"fingerprint"`
}

func (q *Queries) UpdateUserGpgID(ctx context.Context, arg UpdateUserGpgIDParams) error {
	_, err := q.db.ExecContext(ctx, updateUserGpgID, arg.GpgID, arg.Fingerprint)
	return err
}
